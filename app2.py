...
# Set up the OpenAI LLM (using GPT-3.5-turbo for now; switch to gpt-4 if desired)
...
def get_events_for_date_range(start_date, end_date):
    ...
    for _, row in filtered.iterrows():
        ...
        context += f"• {row['Event']} on {event_date_str} at {row['Time']} at {row['Location']}. Price: {price_str}\n"
    return context

# For testing, we simulate "today" as March 13, 2025.
...
# Build dataset context based on query keywords. If query mentions "weekend", show events for Saturday & Sunday.
def build_dataset_context(query):
    ...
    if "weekend" in query_lower or "saturday" in query_lower or "sunday" in query_lower:
        ...
        # Default to events on simulated_today
...
if prompt := st.chat_input("Ask me about Athens events or plan a date:"):
    ...
    # Custom instructions: Tell the assistant to use the dataset as truth,
    # arrange events chronologically, and provide creative suggestions when needed.
    custom_instructions = (
        ...
        "When answering queries, use the dataset as your source of truth, and always arrange events in logical, chronological order. "
        "When a user asks for recommendations—like 'plan a date for me and my girlfriend this weekend'—you should consider the events in the dataset "
