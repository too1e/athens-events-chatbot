...
def determine_target_date(query, base_date):
    ...
    # If query does not include explicit day/date info and there's a stored target date, use that.
    if st.session_state.get("last_target_date") and (
         "that day" in query_lower or not re.search(r"(monday|tuesday|wednesday|thursday|friday|saturday|sunday|\d{1,2}/\d{1,2}/\d{2,4}|weekend|next week)", query_lower)
       ):
...
if prompt := st.chat_input("Ask me about Athens events or plan a date:"):
    ...
    elif "saturday" in query_lower or "sunday" in query_lower or "that day" in query_lower:
    ...
    custom_instructions = (
        ...
        "When answering queries, please base your answer solely on the dataset below and arrange events in strict chronological order "
