...
def get_events_for_date_range(target_date):
    # Filter events for the target date by comparing only the date portion
...
def determine_target_date(query):
    ...
    # If query mentions "next week", add 7 days to simulated_today
    if "next week" in query_lower:
        return simulated_today + timedelta(days=7)
    ...
    if match:
        ...
        for fmt in ("%m/%d/%Y", "%m/%d/%y"):
            try:
                ...
                continue
    # If query mentions "weekend" but not a specific day, default to Saturday
    if "weekend" in query_lower:
        return simulated_today + timedelta(days=(5 - simulated_today.weekday()))
    # Otherwise, default to simulated_today
...
# If a query mentions a specific day, we'll use that; otherwise, if "weekend" is mentioned, use Saturday.
target_date = simulated_today  # default
# --- Main Interaction ---
...
if prompt := st.chat_input("Ask me about Athens events or plan a date:"):
    ...
    # Choose context wording based on query specificity
    if "saturday" in prompt.lower() or "sunday" in prompt.lower():
        ...
        date_context_text = f"for next week, starting {target_date.strftime('%A, %B %d, %Y')}"
    elif "weekend" in prompt.lower():
        # For general "weekend" queries, provide context for Saturday and Sunday
    ...
    custom_instructions = (
        ...
        "(morning events first, then afternoon, then evening; use standard times like '8:00 AM' instead of military time). "
        "When a user asks for recommendations—like 'plan a date for me and my girlfriend this weekend'—select events from the dataset in logical order and blend them with creative suggestions. "
