...
# Set the app title to "The Guide Dawg" (or any name you choose)
st.title("The Guide Dawg")

# Initialize conversation history and last_target_date if not already present
...
def determine_target_date(query, base_date):
    ...
    # If query uses phrases like "that day", "later", "other", or "that night", reuse last target date if available.
...
def build_dataset_context(query, target_date):
    query_lower = query.lower()
    # If the query mentions "next week", build context for the entire week.
    if "next week" in query_lower:
...
if prompt := st.chat_input("Ask me about Athens events or plan a date:"):
    ...
    custom_instructions = (
        ...
        "You're The Guide Dawg—your laid-back events guru with access to the Athens events dataset. "
