...
# Initialize conversation history as a list if not already present
if "messages" not in st.session_state:
    st.session_state.messages = []
# Initialize last_target_date if not present
...
def determine_target_date(query, base_date):
    ...
    # If query uses phrases like "that day", "later", or "other", reuse last target date if available.
    if st.session_state.get("last_target_date") and ("that day" in query_lower or "later" in query_lower or "other" in query_lower):
    ...
    if "next week" in query_lower:
        ...
        return next_monday
    # Check for specific day names (e.g., "wednesday", "thursday")
...
def build_dataset_context(query, target_date):
    query_lower = query.lower()
    # If "next week" is mentioned, build context for the whole week.
    if "next week" in query_lower:
        next_monday = target_date  # target_date here is next Monday from our logic
        next_sunday = next_monday + timedelta(days=6)
        if "karaoke" in query_lower:
            return get_category_events_for_date_range("Karaoke & Open Mic", next_monday, next_sunday)
        elif "music" in query_lower or "concert" in query_lower:
            return get_category_events_for_date_range("Music", next_monday, next_sunday)
        elif "comedy" in query_lower:
            return get_category_events_for_date_range("Comedy", next_monday, next_sunday)
        else:
            return get_events_for_date_range_range(next_monday, next_sunday)
    # Otherwise, if a category is mentioned, filter for that day.
    if "karaoke" in query_lower:
...
if prompt := st.chat_input("Ask me about Athens events or plan a date:"):
    ...
    elif "saturday" in query_lower or "sunday" in query_lower or "that day" in query_lower or "later" in query_lower or "other" in query_lower:
